@extends('layouts.app')
@section('title', 'Edit Izin Operasional Madin')
    ...
    <div class="w-full lg:ps-64">
        <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 sm:p-8 mt-5 dark:bg-slate-900">
                ...
                <form action="{{ route('backsite.madin-operational-permit.update', $madinOperationalPermit->id) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-10"
                        x-data='{
                        "requirements": {{ json_encode(old('data', $madinOperationalPermit->data ?? [''])) }},
                    ...
                    @php
                        ...
                        if ($errors->has('supporting_data.*')) {
                            foreach ($errors->get('supporting_data.*') as $key => $messages) {
                                $supportingDataErrors[$key] = $messages[0];
                            }
                        }
                    @endphp
                    <div class="mb-6"
                        x-data='{
                        "requirements2": {{ json_encode(old('supporting_data', $madinOperationalPermit->supporting_data ?? [''])) }},
                        "errors": {{ json_encode($supportingDataErrors) }},
                        "columnName": "supporting_data"
                    }'>
                        <label
                            class="block text-base font-medium text-gray-800 dark:text-gray-200 mb-5 border-b-2 border-blue-600 pb-2">
                            Dokumen Pendukung <span class="text-red-600">*</span>
                        </label>

                        <template x-for="(requirement, index) in requirements2" :key="index">
                            <div class="mb-3">
                                <div class="flex items-center space-x-2 text-sm">
                                    <input type="text" name="supporting_data[]" x-model="requirements2[index]"
                                        :class="{ 'border-red-500': errors[columnName + '.' + index] }"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-slate-800 dark:border-gray-700 dark:text-gray-300"
                                        placeholder="Contoh: Surat Permohonan Izin">
                    ...
                    <div class="mt-8 flex justify-end items-center space-x-4">
                        <a href="{{ route('backsite.madin-operational-permit.index') }}"
